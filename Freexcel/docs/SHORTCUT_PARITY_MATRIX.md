...
| UI | Shift+F10 / Menu key | Partial | Opens the worksheet context menu with keyboard access keys plus clipboard, Paste Special, cell insert/delete prompts, row/column insert/delete and sort commands that preserve notes and threaded comments, custom sort, filter, clear/reapply filter, pick-from-drop-down-list, Quick Analysis, Define Name, Create Table, Format as Table, Text to Columns, Remove Duplicates, Data Validation dialog option/button access keys, hide/unhide rows and columns, row-height/column-width prompts, AutoFit row height/column width, New Comment routed to the threaded-comment workflow, new/edit/delete/show note actions, hyperlink, format-cells, clear-all, clear-formats, clear-comments for notes and threaded comments, clear-hyperlinks, and clear-content actions. Full Excel context-menu contents and object-aware variants remain partial. |
...
5. Complete the remaining `Alt+Down` Excel visual menu layout and richer nested command UI beyond the current Excel-style filter menu plan with selectable criteria templates.
6. Add full in-grid visual Quick Analysis hover-preview rendering and the remaining Excel gallery options for `Ctrl+Q`; current live preview coverage includes icon-sized menu swatches plus status/target metadata rather than Excel's full rendered worksheet gallery.
