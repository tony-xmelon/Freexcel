...
Recent focused verification: generated XLSX corpus round-trip tests pass; focused chart-format, chart date/category-axis, date-axis time-unit, axis display-unit, axis visibility/delete, axis position, axis tick-label position, reverse-order axis scaling, axis-crossing, category multi-level label, and category label-alignment metadata, PivotTable option/location metadata, PivotChart binding metadata, pivot cache count/version metadata, and drawing-object metadata IO tests pass; and App Host builds successfully. Full-suite counts should be refreshed during the next full parity-report run.
...
| Charts, including radar, stock, bubble, and surface | `generated-charts-001`, `generated-charts-combo-002` with date-category volume stock, bubble option smoke tests, chart protection/print semantic comparison, category/date-axis, date-axis time-unit, axis display-unit, axis visibility/delete, axis position, axis tick-label position, reverse-order axis scaling, axis-crossing, category multi-level label, and category label-alignment metadata, data-label display flags, up/down-bar formatting, smoothed line series, and 2D/3D surface plus floor/wall formatting coverage |
