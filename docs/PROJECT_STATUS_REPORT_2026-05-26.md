...
Mainline observed: `origin/main` at `0dedc1191`
...
| Test lines under `tests/` | 109,274 |
| Documentation lines under `docs/` | 14,734 |
| Test methods marked `[Fact]` / `[Theory]` | 5,586 |
| XLSX corpus manifest rows | 120 |
...
| XLSX fidelity | 120-row manifest baseline; generated, public, local-private, and regression rows drive package retention and semantic comparison. The most recent tester-release validation before this refresh exposed Core.IO corpus regressions; rerun after the active corpus fixes land. |
...
| XLSX corpus | Manifest baseline is 120 rows. Recent failures are now tracked as release blockers rather than documentation drift. |
