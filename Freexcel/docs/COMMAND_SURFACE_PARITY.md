...
| Export to PDF/XPS | Partial | Deterministic PDF export uses the existing print renderer and PDFsharp-WPF raster pages; active-sheet, selected-range, entire-visible-workbook, page-range, page-count validation, standard/minimum-size quality options, ignore-print-areas, extensionless `.pdf`/explicit `.xps` path normalization, access-keyed publish options, open-after-publish options, and PDF-only sheet-name bookmarks with page-range filtering are supported; requested PDF document properties embed normalized workbook-name title plus Freexcel author/subject/keywords metadata, and explicit PDF Info fields are trimmed before writing; XPS export remains available with format-aware option summaries, explicitly reports selected bookmarks as PDF-only, and writes the same modeled title/creator/subject/keywords subset into package core properties; selectable/vector PDF text, heading bookmark variants, and remaining full Excel PDF publish options remain partial |
