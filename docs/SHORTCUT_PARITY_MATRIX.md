...
| File | Ctrl+P | Partial | Opens Freexcel's print preview immediately through the File/Backstage Print entry point, with access-keyed Print, printer, copies, page, and zoom toolbar controls plus an active-sheet print-settings summary for orientation, paper size, scaling, gridlines, headings, and print-area scope. The print preview settings panel exposes access-keyed orientation, paper size, margins, and scaling controls plus keyboardable gridline and heading toggles backed by undoable print option commands. Page Setup exposes tab, option, OK, and Cancel access keys for the modeled print settings. Printed comment output includes notes and local threaded comments for as-displayed overlays and at-end summary pages. Full Excel-style print backstage/settings editing parity remains partial. |
...
| Clipboard | Ctrl+Alt+V | Partial | Opens Paste Special with keyboard focus on the default All choice and access keys for dialog choices and OK/Cancel; implemented modes include values, formulas, formats including transposed format-only paste, values-and-source-formatting, all using source theme, arithmetic operations, arithmetic plus values-and-number-formats, skip blanks, transpose, paste link, picture and linked-picture paste with source-range metadata, comments/notes/threaded-comments, validation, all-except-borders, all-merging-conditional-formats, formulas-and-number-formats, values-and-number-formats, and keep column widths. Full Excel Paste Special option matrix remains partial. |
...
| Formatting | Ctrl+1 | Partial | Opens Format Cells with keyboard access keys for tabs, OK/Cancel, supported option checkboxes, color picker buttons, font/fill/border color swatches, border presets/preview/details controls, and Pivot Value Field Settings tabs when reached from pivot value formatting; covers number, alignment, font, fill, border, and protection fields supported by the current style model, including custom number formats, number/currency/accounting/percentage decimal-place and symbol controls, negative-number variants, shrink-to-fit, indent, rotation, Normal font reset, double underline, superscript/subscript, clear fill, per-edge border style/color, and locked-cell protection. Excel's full multi-page dialog remains broader. |
| Formatting | Ctrl+Shift+F / Ctrl+Shift+P | Partial | Opens Format Cells on the Font tab with keyboard access keys plus font family, size, bold, italic, underline, Normal font reset to the default modeled font, double underline, strikethrough, superscript/subscript, font color picker access, and keyboardable common font-color swatches that update preview and result style. Excel's full font effects surface remains partial. |
...
| Navigation | F5 / Ctrl+G | Parity | Opens Go To for jumping to a cell reference with initial focus and selected text in the reference field, access keys for the reference field, OK, and Cancel; Go To Special exposes access keys for core selection targets and OK/Cancel. |
...
| Review | Shift+F2 / Ctrl+Shift+F2 | Partial | Shift+F2 opens Freexcel's simple note workflow and preloads existing selected-cell note text for editing; Ctrl+Shift+F2 opens a distinct model-backed threaded-comment workflow with undo support, including core command support for editing root threaded-comment text while preserving replies/resolved state, and Ctrl+Shift+O plus Review previous/next/show navigation treat notes and threaded comments as comment-bearing cells. Full Excel threaded conversation/reply UI and XLSX threaded-comment package fidelity remain partial. |
...
| Analysis | Ctrl+Q | Partial | Opens a grouped Quick Analysis menu for formatting, charts, totals, tables, and sparklines using existing Freexcel commands, anchors the keyboard-opened menu to the visible bottom-right of the selected range instead of the mouse point, and moves keyboard focus to the first enabled menu item. It includes conditional-format data bars, color scales, icon sets, greater-than/less-than/between/equal-to/text/date-occurring/duplicate-values, top/bottom item and percent, above/below-average, clear-formatting choices, Column/Stacked Column/100% Stacked Column/Line/Pie/Doughnut/Bar/Stacked Bar/100% Stacked Bar/Area/Scatter/Bubble/Radar/Stock charts plus More Charts to the full Insert Chart dialog, Sum/Average/Count/% Total/Running Total/Max/Min adjacent-row totals, hover preview tooltips, icon-sized visual preview swatches, live mouse and keyboard-focus status previews, and an in-grid preview overlay that targets either the selection or the adjacent totals/sparkline column while preserving command and visual descriptors for rendered worksheet previews. Excel's full rendered worksheet gallery and full option matrix remain partial. |
| Workbook | Ctrl+Shift+G | Parity | Opens Workbook Statistics, with comment totals including notes and threaded comments. |
| UI | F10 | Partial | Enters Freexcel ribbon keytip mode, including when keyboard focus is in text-entry shell regions such as the formula bar or name box; badges are centered near the lower edge of ribbon targets and clamped inside the overlay; pixel-perfect Excel keytip overlay placement remains partial under the broader ribbon keytip row. |
| UI | F6 / Shift+F6 | Partial | Cycles shell focus forward/back through worksheet, ribbon, formula bar, sheet-tab strip, available visible PivotTable Fields or slicer/timeline task panes, and status/zoom controls, with the planner skipping unavailable task panes instead of relying on failed focus attempts. It is handled before text-box filtering so it works from the formula bar, lands on the active sheet tab with tab-strip Left/Right/Home/End navigation, starts status-bar focus at Zoom Out with a slider fallback, keeps Tab/Shift+Tab traversal inside focused status controls and visible task panes instead of moving worksheet cells, returns task-pane focus to the worksheet with Esc, and keeps F6/Shift+F6 inside the visible Backstage overlay instead of moving focus behind File view. Full Excel pane choreography across other task panes, contextual object surfaces, and collapsed ribbon overflow remains partial. |
| UI | Tab / Shift+Tab in ribbon | Partial | The ribbon tab strip and visible ribbon commands are explicit keyboard focus stops, with focused-ribbon Tab/Shift+Tab, arrow keys, and Home/End issuing WPF focus traversal instead of being hijacked by worksheet movement; Esc returns focus to the worksheet. Full Excel ribbon focus choreography across QAT, title bar, ribbon groups, and collapsed overflow remains partial. |
| UI | Shift+F10 / Menu key | Partial | Opens the active-cell-anchored worksheet context menu with keyboard access keys and initial focus on the first enabled menu item, plus clipboard, Paste Special, Insert Copied Cells with shift direction prompt, cell insert/delete prompts, row/column insert/delete and sort commands that preserve notes and threaded comments, custom-sort dialog access keys, filter, clear/reapply filter, pick-from-drop-down-list, Quick Analysis, Define Name, Create Table, Format as Table, Text to Columns, Remove Duplicates, Data Validation dialog option/button access keys, hide/unhide rows and columns, row-height/column-width prompts, AutoFit row height/column width, New/Edit/Delete Comment routed to the threaded-comment workflow, new/edit/delete/show note actions, Excel-like hyperlink state entries for insert/open/edit/remove, format-cells, clear-all, clear-formats, clear-comments for notes and threaded comments, clear-hyperlinks, clear-content actions, target-specific enabled state for comment/note/hyperlink entries, active-cell object-aware menus for picture format/crop/reset-crop, Alt Text, Selection Pane, plus shape/text-box size, rotation, fill, outline, and shape reorder commands, and row/column-selection menus that prune irrelevant column/row layout commands. When F6 focus is on a sheet tab, Shift+F10/Menu opens that sheet-tab context menu and moves focus to the first enabled menu item; when Backstage focus is on a recent/pinned file row, it opens that row's context menu before the worksheet fallback and focuses its first enabled item. Full Excel context-menu contents remain partial. |
...
| Editing | F4 outside formula editing | Partial | Repeats the last repeatable formatting, Merge & Center, paste/paste-special/picture paste, AutoSum, Fill Down/Right/Up/Left, Fill Series, Flash Fill, current date/time insertion, symbol insertion, comments, Clear All/Formats/Contents/Comments/Hyperlinks, sort/filter with notes/threaded-comments preserved, Text to Columns, Remove Duplicates, Data Validation, Consolidate, Subtotal with dialog option access keys, Data Table, Scenario Manager Show, Insert Sheet/Chart/Sparkline/Hyperlink, row-height/column-width dialog results, common chart layout/label/axis/trendline formatting, drawing object insertion/size/rotation/color/reorder/Alt Text, outline group/ungroup/collapse/expand, insert/delete row-column/cell with notes/threaded-comments preserved, or hide/unhide row-column command against the current selection with a fresh undoable command instance. Dialog-driven workflows such as Goal Seek, Scenario Manager save/delete/report, import, protection, and sheet-tab context operations remain intentionally non-repeatable. |
| Ribbon | Alt, then F/H/N/J/P/M/A/R/W/Y; Alt+F/H/N/J/P/M/A/R/W/Y; Alt+1/2/3 | Partial | Opens File backstage or selects the File/Home/Insert/Draw/Page Layout/Formulas/Data/Review/View/Help ribbon tabs through direct Alt combinations or two-step Alt keytip mode, including when focus starts in text-entry shell regions such as the formula bar or name box, and invokes the visible Quick Access Toolbar Save/Undo/Redo commands through direct `Alt+1/2/3` or two-step keytip mode, with File/Backstage focus landing on the Home navigation entry for keyboard continuation, Tab/Ctrl+Tab cycling within the Backstage overlay, Up/Down/Home/End moving through the Backstage sidebar navigation, and repeated recent-file row pin/unpin icon buttons excluded from direct File-scope keytips so visible Backstage badges remain unique while row context menus keep scoped pin/unpin access keys. QAT, tab, formula-bar, sheet-tab, and broad visible command keytip metadata render as visible badges for the current visual tree with measured in-window placement snapped to whole DIPs; contextual PivotTable Analyze/Design top-level keytips `JA`/`JD` route through the same top-level router, and the top-level handler waits on visible longer prefixes so `Alt, J, A` is not shadowed by Draw's `J`. Command-scope overlay measurement refreshes layout after tab switches, QAT badges invoke only from top-level mode, off-tab ribbon controls are filtered out, and visible button/toggle/combo command sequences invoke their controls from command scope with toggle state changes, exact command keytips winning over unrelated longer prefixes, duplicate keytip metadata and deterministic resolver behavior guarded by tests. All direct ribbon dropdown menu items and nested Conditional Formatting menu choices have staged keytip metadata/routing, recursive nested leaf/prefix resolution coverage, dynamic menu keytip assignment repairs duplicate and prefix-conflicting seeded keytips within the menu scope, menu keytips are displayed in the menu gesture-text slot, keyed parent menu choices open their submenu layer before child keytips resolve inside that submenu scope, Escape closes menu keytip mode, and real-window host coverage now exercises Home command, dropdown routing, nested Borders line-color routing, top-level/command overlay badges, plus access-keyed Error Checking and Conditional Formatting manager dialog commands with initial focus on the manager scope selector. Remaining pixel-perfect Excel overlay edge-placement polish and any future nested submenu keytips beyond Conditional Formatting are not complete. |

## Next Shortcut Work

1. Build the full Excel Print backstage/settings editing flow for `Ctrl+P`; the current path opens Freexcel's print preview immediately through File/Backstage Print, with a native print-dialog button and active-sheet settings summary.
...
5. Complete the remaining `Alt+Down` Excel visual menu layout and richer nested command UI beyond the current Excel-style filter menu plan with two-condition custom criteria and keyboard-activatable color swatches.
...
9. Decide which remaining dialog-driven workflows should become repeatable through F4 and add explicit repeat command objects for them.
