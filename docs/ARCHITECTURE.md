...
numeric-placeholder characters inside quoted-affix formats, explicit empty negative/zero positional sections that suppress display, comma scaling, fixed and variable-denominator fractions, date/time, elapsed-time,
...
holds the workbook value. Excel's special `[$-F800]` and `[$-F400]` tokens map to deterministic Freexcel long-date and
long-time patterns for both date values and numeric date serials; OS-localized system date/time pattern selection
remains outside the deterministic formatter boundary. The formatter also maps modeled LCIDs `401`, `402`, `404`, `405`, `406`,
