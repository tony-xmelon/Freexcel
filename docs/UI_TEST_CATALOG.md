...
| UI-CMD-DATA-008 | UI-CAT-DATA-001 | AutoFilter typed criteria and ribbon/keytip routes | AutoFilter dialog now supports the Excel `Alt+Down,F` continuation key by opening the visible text/number/date filter submenu and focusing its first command, and filter-by-color swatches are keyboardable with Tab/Enter/Space plus Arrow/Home/End navigation; remaining work includes full mouse/Alt keytip dropdown creation from headers, checklist navigation, clear/reapply, and row visibility proof. | In Progress |
