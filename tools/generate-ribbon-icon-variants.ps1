...
function L($x1, $y1, $x2, $y2, [int]$n = 20, $cls = 's', $sw = 1) {
    ...
    "  <path class=""$cls"" d=""M$(S $x1 $n) $(S $y1 $n) L$(S $x2 $n) $(S $y2 $n)"" stroke-width=""$(SW $sw)""/>"
}

function Rct($x, $y, $w, $h, [int]$n = 20, $cls = 'f s', $sw = 1) {
    "  <rect class=""$cls"" x=""$(S $x $n)"" y=""$(S $y $n)"" width=""$(S $w $n)"" height=""$(S $h $n)"" stroke-width=""$(SW $sw)""/>"
}
...
function Body($slug, [int]$n) {
    switch -Regex ($slug) {
        ...
        'grow-font|increase-font-size' { return (Txt 'A' 7.5 10.5 13 $n) + "`n" + (L 15 15 15 5 $n 'r' 1) + "`n  <polygon class=""b"" points=""$(S 12.8 $n) $(S 7 $n) $(S 15 $n) $(S 4.5 $n) $(S 17.2 $n) $(S 7 $n)""/>" }
        'shrink-font|decrease-font-size' { return (Txt 'A' 7.5 10.5 10.5 $n) + "`n" + (L 15 5 15 15 $n 'r' 1) + "`n  <polygon class=""b"" points=""$(S 12.8 $n) $(S 13 $n) $(S 15 $n) $(S 15.5 $n) $(S 17.2 $n) $(S 13 $n)""/>" }
        ...
        'increase-decimal' { return (Txt '0.0' 8 7 6.2 $n) + "`n" + (Txt '0.00' 8 13 6.2 $n) + "`n" + (Arrow 13 10 17 10 $n 'b' 1) }
        'decrease-decimal' { return (Txt '0.00' 10 7 6.2 $n) + "`n" + (Txt '0.0' 10 13 6.2 $n) + "`n" + (Arrow 17 10 13 10 $n 'b' 1) }
        'orientation' { return "  <text class=""t"" x=""$(S 9 $n)"" y=""$(S 8.5 $n)"" font-size=""$(S 8 $n)"" transform=""rotate(-45 $(S 9 $n) $(S 8.5 $n))"">ab</text>`n" + (Arrow 5 16 16 5 $n 'b' 1) }
        ...
        '^effects$' { return (Txt $sparkleGlyph 9 8.5 13 $n) + "`n" + (Txt $sparkleGlyph 15 15 6 $n) }
        ...
        '^paper-size$' { return "  <path class=""f s"" d=""M$(S 6 $n) $(S 2.8 $n) H$(S 14 $n) V$(S 17.2 $n) H$(S 6 $n) Z"" stroke-width=""1""/>`n" + (Arrow 4.6 4 4.6 16 $n 'b' .9) + "`n" + (Arrow 15.4 16 15.4 4 $n 'b' .9) }
        ...
        '^conditional-formatting$' { return (Grid $n) + "`n  <rect class=""rd"" x=""$(S 4.5 $n)"" y=""$(S 4.5 $n)"" width=""$(S 4 $n)"" height=""$(S 4 $n)"" opacity="".75""/>`n  <rect class=""y"" x=""$(S 8.5 $n)"" y=""$(S 8.5 $n)"" width=""$(S 4 $n)"" height=""$(S 4 $n)"" opacity="".75""/>`n  <rect class=""g2"" x=""$(S 12.5 $n)"" y=""$(S 12.5 $n)"" width=""$(S 4 $n)"" height=""$(S 4 $n)"" opacity="".85""/>" }
        '^format-as-table$' { return (Grid $n) + "`n  <rect class=""g2"" x=""$(S 3 $n)"" y=""$(S 3 $n)"" width=""$(S 14 $n)"" height=""$(S 3.5 $n)"" opacity="".8""/>`n  <path d=""M$(S 15 $n) $(S 15 $n) L$(S 18 $n) $(S 18 $n)"" fill=""none"" stroke=""#5b9bd5"" stroke-width=""1"" stroke-linecap=""round"" vector-effect=""non-scaling-stroke""/>" }
        '^cell-styles$' { return (Grid $n) + "`n" + (Txt $starGlyph 15 6 5 $n) + "`n  <rect class=""y"" x=""$(S 6.5 $n)"" y=""$(S 6.5 $n)"" width=""$(S 7 $n)"" height=""$(S 7 $n)"" opacity="".45""/>" }
        ...
        '^more-functions$' { return (Txt $sigmaGlyph 6.5 10 9 $n) + "`n" + (Txt '...' 14 10 8 $n) }
        '^define-name$|^use-in-formula$|^create-from-selection$' { return "  <path class=""f s"" d=""M$(S 3 $n) $(S 6 $n) H$(S 11.5 $n) L$(S 16.5 $n) $(S 10 $n) L$(S 11.5 $n) $(S 14 $n) H$(S 3 $n) Z"" stroke-width=""1""/>`n  <circle class=""b"" cx=""$(S 12 $n)"" cy=""$(S 10 $n)"" r=""$(S 1.5 $n)""/>" }
        '^name-manager$' { return (Doc $n) + "`n" + (Txt 'fx' 9.5 8.5 7 $n) + "`n" + (L 6 14 14 14 $n 'b' 1) }
        '^trace-precedents$' { return (Txt 'fx' 14 10 7 $n) + "`n" + (Arrow 3.5 10 10 10 $n 'b' 1) }
        '^trace-dependents$' { return (Txt 'fx' 6 10 7 $n) + "`n" + (Arrow 10 10 16.5 10 $n 'b' 1) }
        '^show-formulas$' { return (Txt '=' 6 8 9 $n) + "`n" + (Txt 'fx' 12 12 8 $n) }
        '^error-checking$' { return "  <path class=""y s"" d=""M$(S 10 $n) $(S 3 $n) L$(S 17 $n) $(S 16 $n) H$(S 3 $n) Z"" stroke-width=""$(S 1 $n)""/>`n" + (Txt '!' 10 11 8 $n) }
        '^watch-window$' { return (WindowIcon $n) + "`n" + (Txt 'fx' 10 10 7 $n) }
        ...
        '^fill$' { return "  <rect class=""f s"" x=""$(S 7 $n)"" y=""$(S 3 $n)"" width=""$(S 6 $n)"" height=""$(S 14 $n)"" stroke-width=""1""/>`n" + (L 7 7 13 7 $n 's' 1) + "`n" + (L 7 11 13 11 $n 's' 1) + "`n" + (Txt $boltGlyph 15 10 8 $n) }
        '^clear$' { return "  <path class=""f s"" d=""M$(S 6 $n) $(S 13 $n) L$(S 13 $n) $(S 6 $n) L$(S 17 $n) $(S 10 $n) L$(S 10 $n) $(S 17 $n) Z"" stroke-width=""1""/>`n" + (L 4 17 17 17 $n 's' 1) }
        '^sort$' { return (Txt 'A' 6 6.5 6 $n) + "`n" + (Txt 'Z' 6 14 6 $n) + "`n" + (Arrow 13.5 15.5 13.5 4.5 $n 'g' 1) + "`n" + (Funnel $n) }
        '^get-data$' { return (Database $n) + "`n" + (Arrow 14 15 17 12 $n 'g' .9) }
        '^refresh-all$' { return (Database $n) + "`n  <path d=""M$(S 14 $n) $(S 7 $n) A$(S 5 $n) $(S 5 $n) 0 1 0 $(S 15 $n) $(S 13 $n)"" fill=""none"" stroke=""#5b9bd5"" stroke-width=""1"" stroke-linecap=""round"" vector-effect=""non-scaling-stroke""/>`n" + (Arrow 13 5 16 7 $n 'g' .9) }
        '^clear-filter$' { return (Funnel $n) + "`n  <path d=""M$(S 6 $n) $(S 6 $n) L$(S 15 $n) $(S 15 $n) M$(S 15 $n) $(S 6 $n) L$(S 6 $n) $(S 15 $n)"" fill=""none"" stroke=""#d83b01"" stroke-width=""1"" stroke-linecap=""round"" vector-effect=""non-scaling-stroke""/>" }
        ...
        '^flash-fill$' { return "  <rect class=""f s"" x=""$(S 7 $n)"" y=""$(S 3 $n)"" width=""$(S 6 $n)"" height=""$(S 14 $n)"" stroke-width=""1""/>`n" + (L 7 7 13 7 $n 's' 1) + "`n" + (L 7 11 13 11 $n 's' 1) + "`n" + (Txt $boltGlyph 15 10 9 $n) }
        ...
        '^subtotal$' { return (Txt $sigmaGlyph 6 7 8 $n) + "`n" + (L 10 6 17 6 $n 's' 1) + "`n" + (L 4 11 17 11 $n 's' 1) + "`n" + (L 4 15 17 15 $n 's' 1) }
        ...
        '^bring-forward$' { return "  <rect class=""b"" x=""$(S 4.5 $n)"" y=""$(S 8 $n)"" width=""$(S 8.5 $n)"" height=""$(S 8.5 $n)""/>`n" + (Rct 8 4 8.5 8.5 $n) }
        '^send-backward$' { return (Rct 4.5 8 8.5 8.5 $n) + "`n  <rect class=""b"" x=""$(S 8 $n)"" y=""$(S 4 $n)"" width=""$(S 8.5 $n)"" height=""$(S 8.5 $n)""/>" }
        '^object-size$|^size$' { return (Rct 5 6 10 8 $n) + "`n" + (Arrow 4 16 16 16 $n 'b' .9) + "`n" + (Arrow 17 15 17 5 $n 'b' .9) }
        ...
        '^zoom-in$' { return "  <circle class=""r"" cx=""$(S 8.5 $n)"" cy=""$(S 8.5 $n)"" r=""$(S 5 $n)"" stroke-width=""1""/>`n" + (L 8.5 5.5 8.5 11.5 $n 'r' 1) + "`n" + (L 5.5 8.5 11.5 8.5 $n 'r' 1) + "`n" + (L 12 12 17 17 $n 'r' 1) }
        '^zoom-out$' { return "  <circle class=""r"" cx=""$(S 8.5 $n)"" cy=""$(S 8.5 $n)"" r=""$(S 5 $n)"" stroke-width=""1""/>`n" + (L 5.5 8.5 11.5 8.5 $n 'r' 1) + "`n" + (L 12 12 17 17 $n 'r' 1) }
        ...
        '^what-s-new$' { return (Txt $sparkleGlyph 8.5 8 11 $n) + "`n" + (Txt '!' 14 13 8 $n) }
        ...
        '^show-notes$' { return "  <path class=""y s"" d=""M$(S 4 $n) $(S 5 $n) H$(S 16 $n) V$(S 13 $n) H$(S 10 $n) L$(S 6 $n) $(S 16 $n) V$(S 13 $n) H$(S 4 $n) Z"" stroke-width=""1""/>`n" + (L 7 8 13 8 $n 's' 1) + "`n" + (L 7 11 12 11 $n 's' 1) }
        '^protect-sheet$' { return (Grid $n) + "`n  <path class=""f s"" d=""M$(S 7 $n) $(S 10 $n) H$(S 15 $n) V$(S 17 $n) H$(S 7 $n) Z M$(S 8.5 $n) $(S 10 $n) V$(S 8 $n) C$(S 8.5 $n) $(S 5.5 $n) $(S 13.5 $n) $(S 5.5 $n) $(S 13.5 $n) $(S 8 $n) V$(S 10 $n)"" stroke-width=""1""/>" }
        '^protect-workbook$' { return "  <path class=""f s"" d=""M$(S 4 $n) $(S 6 $n) C$(S 7 $n) $(S 5 $n) $(S 9 $n) $(S 6 $n) $(S 10 $n) $(S 7 $n) C$(S 11 $n) $(S 6 $n) $(S 13 $n) $(S 5 $n) $(S 16 $n) $(S 6 $n) V$(S 16 $n) C$(S 13 $n) $(S 15 $n) $(S 11 $n) $(S 16 $n) $(S 10 $n) $(S 17 $n) C$(S 9 $n) $(S 16 $n) $(S 7 $n) $(S 15 $n) $(S 4 $n) $(S 16 $n) Z"" stroke-width=""1""/>`n  <path class=""f s"" d=""M$(S 8 $n) $(S 11 $n) H$(S 15 $n) V$(S 17 $n) H$(S 8 $n) Z M$(S 9.2 $n) $(S 11 $n) V$(S 9.5 $n) C$(S 9.2 $n) $(S 7.5 $n) $(S 13.8 $n) $(S 7.5 $n) $(S 13.8 $n) $(S 9.5 $n) V$(S 11 $n)"" stroke-width=""1""/>" }
