...
| PivotChart XLSX parity slice | Pass; bound PivotCharts can be authored from PivotTables, refresh with the PivotTable materialized output range, support undoable type changes that preserve the binding, read/write chart `pivotSource` name and format-id metadata, round-trip PivotChart `pivotFmts`, chart external-data relationship pointers plus package relationship type/target/target-mode metadata, plot-area and legend manual layout metadata, 3D view plus floor/wall formatting metadata, date-system/language, color-map overrides, print settings, style ids, protection flags, rounded-corner, auto-title-deleted, hidden-row-data visibility, blank-display, rendered data-table options, chart/plot/legend/axis title/axis label/date-axis/date-axis time units/axis built-in/custom display units/axis visibility/delete flags/axis position metadata/axis tick-label positions/axis reverse-order scaling/log-axis base metadata/axis-crossing/category multi-level label/category label alignment/data-label/trendline/error-bar/guide-line/up-down-bar visual formatting, per-series smoothed-line and invert-if-negative flags, series-line formatting, and data-label-over-maximum chart-space metadata from chart package design metadata, expose editable PivotChart Options for field buttons, data-table/legend-key display, rounded corners, hidden-row data visibility, and blank-cell display mode, and native same-sheet/cross-sheet PivotChart package graphs resolve back to their PivotTable cache binding after load/save |
...
| Native chart family expansion | Pass for authored/read combo, radar, stock high-low-close/open-high-low-close/volume stock package parts, bubble chart scale/negative-bubble/size-representation options, volume stock rendering, date-axis stock rendering with date-axis time-unit metadata, value-axis built-in/custom display-unit metadata, axis visibility/delete metadata, axis position metadata, axis tick-label position metadata, reverse-order axis scaling metadata, log-axis base metadata, axis crossing/cross-between metadata, category multi-level label and label-alignment metadata, stock up/down-bar candlestick rendering with up/down bar fill/border/gap metadata, line/area blank-display rendering plus blank-as-zero column/bar rendering, line/scatter per-series smoothed-line metadata, bar/column invert-if-negative metadata, stacked bar/column series-line metadata, 2D/3D surface `surfaceChart`/`surface3DChart` package parts with series axes, value-colored matrix rendering paths, and 3D floor/wall formatting metadata, 3D clustered column/bar `bar3DChart`, 3D line `line3DChart`, 3D area `area3DChart`, and 3D pie `pie3DChart` package/rendering paths, alongside existing supported chart families; true 3D mesh-style surface polish plus histogram, waterfall, treemap, sunburst, box-whisker, funnel, and map are explicitly detected as unsupported chart families and stay in the retention/warning path |
...
| Text box and shape XLSX fidelity | Pass for native/authored text boxes, basic rectangle/ellipse/line drawing shapes, nonvisual title/description metadata, and retained native connector/group-shape anchors alongside Freexcel-authored drawing objects |
...
Recent focused verification: generated XLSX corpus round-trip tests pass; focused chart-format, chart date/category-axis, date-axis time-unit, axis built-in/custom display-unit, axis visibility/delete, axis position, axis tick-label position, reverse-order axis scaling, log-axis base metadata, axis-crossing, category multi-level label, and category label-alignment metadata, PivotTable option/location metadata, PivotChart binding metadata, pivot cache count/version metadata, and drawing-object metadata IO tests pass; and App Host builds successfully. Full-suite counts should be refreshed during the next full parity-report run.
...
| Charts, including radar, stock, bubble, and surface | `generated-charts-001`, `generated-charts-combo-002` with date-category volume stock, bubble option smoke tests, chart protection/print semantic comparison, category/date-axis, date-axis time-unit, axis built-in/custom display-unit, axis visibility/delete, axis position, axis tick-label position, reverse-order axis scaling, log-axis base metadata, axis-crossing, category multi-level label, and category label-alignment metadata, data-label display flags, up/down-bar formatting, smoothed line series, and 2D/3D surface plus floor/wall formatting coverage |
