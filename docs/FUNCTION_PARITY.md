...
Post-sweep hardening also pins `ASC`, `DBCS`, and `JIS` to Excel's non-DBCS language behavior: width conversion is only applied for DBCS cultures, while English/non-DBCS settings return the original text unchanged.
