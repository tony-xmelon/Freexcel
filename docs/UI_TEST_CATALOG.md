...
| Documented shortcut rows | 85 | From `SHORTCUT_PARITY_MATRIX.md`: 72 parity, 13 partial. |
...
|---|---|
| Shortcut/keytip routing | 85 documented shortcut rows; 68 matcher rules; 67 dispatcher targets; broad XAML keytip metadata including top-level `F/H/N/J/P/M/A/R/W/Y` plus contextual PivotTable `JA/JD`; representative nested menu keytip coverage exists, but all 13 partial shortcut rows still need hands-on UI passes. |
