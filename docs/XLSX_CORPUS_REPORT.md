...
| Native chart family expansion | Pass for authored/read combo, radar, stock high-low-close/open-high-low-close/volume stock package parts, volume stock rendering, date-axis stock rendering, stock up/down-bar candlestick rendering, line/area blank-display rendering plus blank-as-zero column/bar rendering, surface `surfaceChart` package and matrix rendering paths, 3D clustered column/bar `bar3DChart`, 3D line `line3DChart`, 3D area `area3DChart`, and 3D pie `pie3DChart` package/rendering paths, alongside existing supported chart families; 3D mesh-style surface polish plus histogram, waterfall, treemap, sunburst, box-whisker, funnel, and map are explicitly detected as unsupported chart families and stay in the retention/warning path |
