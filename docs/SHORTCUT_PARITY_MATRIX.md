...
| UI | Tab / Shift+Tab in ribbon | Partial | The ribbon tab strip and visible ribbon commands are explicit keyboard focus stops, with focused-ribbon Tab/Shift+Tab and arrow keys issuing WPF focus traversal instead of being hijacked by worksheet movement; Esc returns focus to the worksheet. Full Excel ribbon focus choreography across QAT, title bar, ribbon groups, and collapsed overflow remains partial. |
