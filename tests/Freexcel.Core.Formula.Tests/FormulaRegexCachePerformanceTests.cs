...
public sealed class FormulaRegexCachePerformanceTests
{
    ...
    {
        var source = File.ReadAllText(FindWorkspaceFile("src", "Freexcel.Core.Formula", "ExcelTextNumberParser.cs"));
    ...
    {
        var source = File.ReadAllText(FindWorkspaceFile("src", "Freexcel.Core.Formula", "BuiltInFunctions.DateTime.cs"));
