...
- PivotTable functional core is implemented, including creation, refresh, field layout/source/options changes, filtering/grouping/sorting, Show Values As, calculated fields/items, GETPIVOTDATA, Show Details, PivotChart sync, slicer/timeline integration, external/OLAP pivot-cache source metadata load/save, and custom PivotStyle definition metadata load/save. Remaining gaps are exact PivotStyle gallery UI/rendering semantics, full PivotChart layout/design editing, and external/OLAP/data-model refresh or execution.
