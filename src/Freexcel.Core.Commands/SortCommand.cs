...
public sealed class SortCommand : IWorkbookCommand
{
    ...
    public CommandOutcome Apply(ICommandContext ctx)
    {
        ...
        rows.Sort((a, b) =>
        {
            ...
            {
                var cmp = SortValueComparer.CompareKey(ctx.Workbook, a.Cells[index], b.Cells[index], sortOn, targetColor, _options.CaseSensitive);
                if (cmp != 0)
        ...
        int colCount)
    {
        ...
        columns.Sort((a, b) =>
        {
            ...
            {
                var cmp = SortValueComparer.CompareKey(workbook, a.Cells[index], b.Cells[index], sortOn, targetColor, _options.CaseSensitive);
                if (cmp != 0)
                    return ascending ? cmp : -cmp;
            }
