...
6. Add full in-grid visual Quick Analysis hover-preview rendering and the remaining Excel gallery options for `Ctrl+Q`; current live preview coverage includes icon-sized menu swatches plus status/target metadata and a More Charts path rather than Excel's full rendered worksheet gallery.
...
8. Expand `Shift+F10` / Menu key toward the remaining full Excel worksheet context menu entries and object-aware variants beyond the current command-backed worksheet menu with copied-cell insertion.
