...
| UI-CAT-FILE-001 | Backstage navigation | File Home/Info/New/Open/Save/Save As/Print/Export/Share/Account/Options/Close/Back. | Mouse, Alt+F keytips, Tab/Ctrl+Tab containment, access keys, Escape, sidebar Up/Down/Home/End traversal, Shift+F10 recent/pinned row menus, UIA invoke. | Unsaved, saved local path, missing recent file, unsupported-feature workbook. | Correct page/dialog opens, File/Backstage focus lands on Home navigation for keyboard continuation, disabled/excluded commands explain state, Escape/Back returns focus to workbook. | In Progress |
