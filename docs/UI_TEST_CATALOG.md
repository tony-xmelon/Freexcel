...
|---|---:|---|
| Command surface in-scope rows | 186 | From `COMMAND_INVENTORY.json`: Implemented + Partial command-surface rows. |
| Menu/toolbar in-scope rows | 187 | Includes the current Draw tab menu/toolbar delta. |
...
| XAML click-wired controls | 612 | `Click="..."` occurrences in `MainWindow.xaml` on latest synced `origin/main`. |
...
| UI-CMD-HARNESS-001 | UI-CAT-SHELL-001 | Screenshot and visual evidence harness | Use `tools/screenshot_excel.ps1` and `tools/screenshot_ribbon.ps1` against latest build; verify foreground guard, captured window bounds, ribbon-tab screenshots, popup/dropdown limitations, output naming, and catalog evidence attachment. Use the in-app no-live-Excel ribbon tour with `FREEX_SS_TOUR=1`, optional `FREEX_SS_TOUR_TABS` and `FREEX_SS_TOUR_WIDTHS`, for the default 36 planned captures across the catalog-backed ribbon tab set. Use `FREEX_SS_TOUR_BURST=1` to expand the same tour into 108 burst-phase captures named with `immediate`, `first_render`, and `settled` suffixes so resize/tab-switch flashes can be inspected without foreground automation. Source tests now prove both scripts declare the guarded ribbon tab set, the FreeX script matches the CI-safe tour planner, the tour planner matches the XAML catalog snapshot, normal captures are uniquely named, burst captures cover immediate/first-render/settled phases, scripts capture from window bounds, write deterministic `excel_*/ribbon_*` output names, and emit `screenshot_manifest.json` metadata for catalog evidence plus popup/dropdown limitations. | In Progress |
