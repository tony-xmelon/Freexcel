...
public sealed class XlsxFileAdapterPerformanceTests
{
    ...
    public void Benchmark_LoadExternalWorkbook_ReportsTiming()
    {
        ...
        if (paths.Length == 0)
        {
            Console.WriteLine("PERF XLSX_LOAD_EXTERNAL skipped=true reason=FREEXCEL_IO_BENCHMARK_PATHS_NOT_SET");
            return;
        }
    ...
    {
        var configured = Environment.GetEnvironmentVariable("FREEXCEL_IO_BENCHMARK_PATHS");
        if (string.IsNullOrWhiteSpace(configured))
            return [];

        var limit = 3;
        if (int.TryParse(Environment.GetEnvironmentVariable("FREEXCEL_IO_BENCHMARK_LIMIT"), out var configuredLimit))
