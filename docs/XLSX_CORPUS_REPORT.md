...
Recent focused verification: generated XLSX corpus round-trip tests pass; focused chart-format, chart date/category-axis, date-axis time-unit, axis built-in/custom display-unit, axis custom number-format metadata, axis visibility/delete, axis position, axis tick-label position, reverse-order axis scaling, log-axis base metadata, axis-crossing, category multi-level label, and category label-alignment metadata, PivotTable option/location metadata, PivotChart binding metadata, pivot cache count/version metadata, and drawing-object metadata IO tests pass; and App Host builds successfully. Full-suite counts should be refreshed during the next full parity-report run.
