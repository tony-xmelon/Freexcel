...
| Structured tables (load/save; native parts retained) | Partial | Metadata, style flags, totals-row labels/functions, simple value AutoFilter metadata, authored table parts, and native references are retained; command-level and XLSX-loaded table value filters apply row visibility for multi-column checked-item filters while excluding totals rows; totals-row labels and common functions can be materialized with undo; basic data-body column structured references such as `Sales[Amount]` evaluate through formula functions and register recalculation dependencies; rich structured-reference selectors and full table style galleries remain partial |
