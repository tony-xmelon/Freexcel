...
| UI | F6 / Shift+F6 | Partial | Cycles shell focus forward/back through worksheet, ribbon, formula bar, sheet-tab strip, and status/zoom controls, and is handled before text-box filtering so it works from the formula bar. Full Excel pane choreography across task panes, contextual object surfaces, and collapsed ribbon overflow remains partial. |
