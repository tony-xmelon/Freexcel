...
| UI-CMD-VIEW-004 | UI-CAT-VIEW-002 | Zoom, Zoom to Selection, 100%, Arrange All | View > Zoom preset keytips and View > Arrange All keytips mutate model/status state and exit keytip mode, including checked arrangement state refresh. Remaining work includes status/ribbon shortcut breadth, Zoom dialog custom percent selected on open, Zoom to Selection, and focus return evidence. | In Progress |
