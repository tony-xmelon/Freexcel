...
| Native chart family expansion | Pass for authored/read combo, radar, stock high-low-close/open-high-low-close/volume stock package parts, volume stock rendering, date-axis stock rendering, stock up/down-bar candlestick rendering, 3D clustered column/bar `bar3DChart`, and 3D pie `pie3DChart` package/rendering paths, alongside existing supported chart families; surface, histogram, waterfall, treemap, sunburst, box-whisker, funnel, and map are explicitly detected as unsupported chart families and stay in the retention/warning path |
