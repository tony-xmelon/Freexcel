...
| UI-CMD-INSERT-009 | UI-CAT-INSERT-003 | Header & Footer, Symbol, Hyperlink | Dialog access keys, token buttons, Ctrl+K, cancel/apply, persistence. Symbol picker source coverage now proves access keys, symbol/special-character tabs, initial grid focus, UIA names for symbols and special characters, Unicode character-code parsing/rejection, recent-symbol promotion, double-click/Insert acceptance, cancel, and host insertion of the selected symbol string into the active cell. | In Progress |
