...
| UI-CMD-PAGE-002 | UI-CAT-PAGE-001 | Background, Print Titles, Scale to Fit | Print Titles opens Page Setup directly on the Sheet tab with keyboard focus/select-all on Rows to repeat at top; remaining work includes native image dialog guard, broader range picker execution, page-layout render, Scale to Fit command routes, and persistence expectations. | In Progress |
...
| UI-CMD-REVIEW-002 | UI-CAT-REVIEW-001 | Accessibility Checker and Statistics | Issue list, focus target action, low-contrast cell-text issues, chart alt/title issues, hidden content, comment counts. | In Progress |
| UI-CMD-REVIEW-003 | UI-CAT-REVIEW-002 | Notes and threaded comments | New/edit/delete/previous/next/show, Shift+F2/Ctrl+Shift+F2, persistence limits; live host keytip coverage now proves `Alt,R,N` and `Alt,R,PN` navigate across plain notes and threaded comments. | In Progress |
...
| UI-CMD-DATA-008 | UI-CAT-DATA-001 | AutoFilter typed criteria and ribbon/keytip routes | AutoFilter dialog now supports the Excel `Alt+Down,F` continuation key by opening the visible text/number/date filter submenu and focusing its first command, filter-by-color swatches are keyboardable with Tab/Enter/Space plus Arrow/Home/End navigation, and typed criteria now has dialog-to-filter-command row visibility proof. Remaining work includes full mouse/Alt keytip dropdown creation from headers, checklist navigation, and clear/reapply. | In Progress |
...
| UI-CMD-REVIEW-006 | UI-CAT-REVIEW-002 | Protection permissions and command disabled-state matrix | Protect sheet/workbook with selected permissions, Allow Edit Ranges disabled while the active sheet is protected and blocked from Review keytip invocation, locked/unlocked cells, password/cancel/invalid flows, ribbon/context/menu disabled states, edit attempts by target type, unprotect, undo limits, and persistence. | In Progress |
...
| UI-CMD-CONTEXT-006 | UI-CAT-CONTEXT-001 | Target-specific worksheet context coverage | Planner coverage now verifies worksheet, row, column, picture, shape, and text-box context menus expose only their expected command families. Live Shift+F10/Menu host coverage now proves worksheet, whole-row, whole-column, and picture-scoped routing. Remaining work includes live right-click/Menu routing for shape/text-box plus table, filtered range, chart, PivotTable, protected sheet, and edit-mode enabled states. | In Progress |
