...
| Structured tables (load/save; native parts retained) | Partial | Metadata, style flags, totals-row labels/functions, simple value AutoFilter metadata, authored table parts, and native references are retained; command-level table value filters apply row visibility for multi-column checked-item filters; structured-reference formulas, totals-row calculations, and full XLSX-load filter materialization remain partial |
