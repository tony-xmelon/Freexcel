...
| UI | F6 / Shift+F6 | Partial | Cycles shell focus forward/back through worksheet, ribbon, formula bar, sheet-tab strip, visible PivotTable Fields task pane, and status/zoom controls, is handled before text-box filtering so it works from the formula bar, lands on the active sheet tab with tab-strip Left/Right/Home/End navigation, starts status-bar focus at Zoom Out with a slider fallback, keeps Tab/Shift+Tab traversal inside focused status controls and the visible PivotTable Fields task pane instead of moving worksheet cells, returns task-pane focus to the worksheet with Esc, and keeps F6/Shift+F6 inside the visible Backstage overlay instead of moving focus behind File view. Full Excel pane choreography across other task panes, contextual object surfaces, and collapsed ribbon overflow remains partial. |
