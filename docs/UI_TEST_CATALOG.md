...
| UI-CMD-CONTEXT-006 | UI-CAT-CONTEXT-001 | Target-specific worksheet context coverage | Planner coverage now verifies worksheet, row, column, picture, shape, and text-box context menus expose only their expected command families. Remaining work includes live right-click/Menu/Shift+F10 routing plus table, filtered range, chart, PivotTable, protected sheet, and edit-mode enabled states. | In Progress |
