...
| PivotTable + pivot-cache metadata (load/save; native parts retained) | Partial | Creation, refresh, undoable command-level field layout changes, package retention, authored pivot package parts, values-only and column-only layouts, multiple row/column/value fields, nested column-field matrices, common summaries, single/multi-select page filters, date/number grouping, row/column label filters, row/column value filters with field targets, value/label sorting including column-label sorting, separate row/column grand-total visibility, repeated-label suppression, blank-line spacing, PivotTable style names, subtotals, calculated fields/items, ribbon/double-click Show Details drill-down sheets for item/subtotal/grand-total/matrix/column-only cells, and PivotChart sync are implemented; full drag/drop field-list pane, richer filters/layouts, and complete style rendering are deferred |
...
- PivotTable metadata load/save, native package retention, authored pivot package parts, creation, refresh, undoable command-level field layout editing, values-only and column-only layouts, multiple row fields, multiple column fields with nested matrix headers, multiple data fields, common summary functions, single/multi-select page-field filtering, date/number grouping, row/column label filters, row/column value filters with field-target round-trip, value/label sorting including column-label sorting, separate row/column grand-total visibility, repeated-label suppression, blank-line spacing, style-name round-trip, subtotals, calculated fields/items, ribbon/double-click Show Details drill-down detail-sheet creation for item/subtotal/grand-total/matrix/column-only cells, and PivotChart output-range sync are implemented. Full drag/drop field-list pane, richer advanced filters, slicer/timeline filtering UI, and complete visual style rendering remain deferred.
