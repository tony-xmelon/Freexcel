...
`PivotTableModel.ShowItemsWithNoDataOnColumns` materializes column-field items from PivotCache shared items even when
the current source rows have no matching records, allowing no-data columns to display the configured empty-cell text.
