...
| Last updated timestamp | 2026-05-26T10:14:33+03:00 |
...
| Intake owner | Codex |
| Working branch | codex/user-feedback-batch-2 |
...
| UT-2026-05-26-015 | Inserted comment has a marker but no visible comment panel or hover display | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by code path | Medium | P1 | Verified | Root cause: comment markers were rendered from a boolean flag, but selection and normal hover paths never surfaced the note/thread text. Resolution adds a cell comment preview formatter and updates the grid tooltip on selected or hovered comment cells. | 2026-05-26T10:14:33+03:00 | Added `CommentNavigationPlannerTests.FormatCellCommentPreview_ShowsNotesAndThreadedCommentsForHoveredCell` and `MainWindowSourceHygieneTests.Selection_UpdatesVisibleCommentPreviewForSelectionAndHover`; focused host tests passed: 148/148. |
| UT-2026-05-26-016 | Inserted charts are not displaying | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by code path | High | P1 | Verified | Root cause: chart rendering built series from only the visible viewport cells, so inserted charts could render blank when their source data was offscreen or on another source sheet. Resolution adds chart source-data cells to the viewport model and has the renderer prefer those cells. | 2026-05-26T10:14:33+03:00 | Added `ChartRendererTests.ColumnRenderer_UsesChartDataCellsWhenSourceRangeIsOutsideVisibleViewport`; focused UI chart tests passed: 78/78. |
| UT-2026-05-26-017 | Pasting cells from Excel creates a selected object instead of cell values | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by code path | High | P0 | Verified | Root cause: normal paste tried clipboard images before reading external text; Excel cell copies often expose both tabular text and a rendered image/metafile, so Freexcel inserted a picture-like object. Resolution makes normal paste prefer non-empty clipboard text and keeps image paste for image-only clipboards or explicit paste-as-picture. | 2026-05-26T10:14:33+03:00 | Added `ClipboardPastePlannerTests.ShouldPasteClipboardImageForNormalPaste_PrefersTabularTextOverImage`; focused host tests passed: 148/148. |
| UT-2026-05-26-018 | Insert Link dialog accepts data but no hyperlink persists | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by code path | Medium | P1 | Verified | Root cause: hyperlink model state was written, but the cell retained normal styling, making the inserted link appear as ordinary text and easy to interpret as not persisted. Resolution applies visible hyperlink styling (theme hyperlink color and underline) while preserving undo behavior. | 2026-05-26T10:14:33+03:00 | Added `HyperlinkCommandTests.SetHyperlinkCommand_AppliesVisibleHyperlinkStyle`; focused model tests passed: 7/7. |
| UT-2026-05-26-019 | Font size dropdown selection does not apply to selected cell | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by code path | Medium | P1 | Verified | Root cause: the editable dropdown read stale `ComboBox.Text` during selection changes and bypassed the shared font-size path used by increase/decrease buttons. Resolution reads the selected dropdown item first, routes valid values through `ApplyFontSizeAndFitRows`, and refreshes toolbar state after row-height adjustment. | 2026-05-26T10:14:33+03:00 | Added `MainWindowSourceHygieneTests.FontSizeDropdown_UsesSharedFontSizeApplyPath`; focused host tests passed: 148/148. |
| UT-2026-05-26-020 | Saved file cannot be reopened | 2026-05-26T09:49:44+03:00 | User feedback | Windows x64 single-file build | Reproduced by save-path review; original file not available | Critical | P0 | Verified | Root cause candidates: native `.fxl` save/open adapter coverage was missing from real format resolution tests, and XLSX package preservation snapshots were not refreshed after a successful save. Resolution adds native save/reopen smoke coverage and refreshes the XLSX source-package snapshot after post-processing so repeated saves preserve from the newly written package. | 2026-05-26T10:14:33+03:00 | Added `NativeJsonAdapter_SaveThenResolveOpenAdapterAndReload` and expanded real adapter filter tests to cover `.fxl`; focused IO tests passed: 25/25. Exact user file still needed for a direct repro. |
...
| 2026-05-25T13:13:07+03:00 | Began closing coverage gaps with `HeavyWorkbookRetestTests`, an environment-aware real-workbook open/save smoke test. It passed 1/1 against `E:\Users\anton\Documents\Melon\Kin+Carta\Partner Dashboard 20250116.xlsx` and never modifies the source workbook. |
| 2026-05-26T09:49:44+03:00 | Filed six follow-up user feedback issues UT-2026-05-26-015 through UT-2026-05-26-020 covering comment hover/panel display, chart visibility, Excel clipboard paste routing, hyperlink persistence, font-size dropdown application, and save/reopen failure. Started subagent investigation on `codex/user-feedback-batch-2`. |
