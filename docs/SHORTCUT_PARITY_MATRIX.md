...
| Formulas | F4 while editing a formula reference | Partial | Cycles local, local range, full-column, lowercase-normalized, sheet-qualified, escaped quoted-sheet, 3D sheet-range, and external-workbook A1 references through relative/absolute modes, while preserving structured-reference column names and string literals. |
